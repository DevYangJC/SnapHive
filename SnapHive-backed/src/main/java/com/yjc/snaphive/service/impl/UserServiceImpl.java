...
import com.yjc.snaphive.constant.CommonValue;
...
import com.yjc.snaphive.esdao.EsPostDao;
import com.yjc.snaphive.esdao.EsUserDao;
...
import com.yjc.snaphive.manager.FileManager;
import com.yjc.snaphive.manager.auth.StpKit;
import com.yjc.snaphive.mapper.UserMapper;
import com.yjc.snaphive.mapper.UserSignInRecordMapper;
import com.yjc.snaphive.model.dto.file.UploadPictureResult;
import com.yjc.snaphive.model.dto.user.UserModifyPassWord;
import com.yjc.snaphive.model.dto.user.UserQueryRequest;
import com.yjc.snaphive.model.dto.user.UserExportRequest;
...
import com.yjc.snaphive.model.enums.UserRoleEnum;
import com.yjc.snaphive.model.vo.LoginUserVO;
...
import com.yjc.snaphive.service.PostService;
import com.yjc.snaphive.service.PostAttachmentService;
import com.yjc.snaphive.utils.EmailSenderUtil;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public long userRegister(String email, String userPassword, String checkPassword, String code) {
        ...
        if (correctCode == null || !correctCode.equals(code)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "验证码错误或已过�?);
        }
        ...
        synchronized (email.intern()) {
            ...
            user.setUserName(userAccount); // 使用账号作为默认用户�?
    ...
    public LoginUserVO userLogin(String accountOrEmail, String userPassword, HttpServletRequest request) {
        ...
        // 4. 记录用户登录�?
        // 先设�?Sa-Token 登录态，这样可以确保 Session 中有正确的权限信�?
        StpKit.SPACE.login(user.getId());
        // �?Sa-Token Session 中存入完整的用户信息
        StpKit.SPACE.getSession().set(UserConstant.USER_LOGIN_STATE, user);
        // �?Spring Session 中也存入用户信息（为了兼容旧代码�?
    ...
    public User getLoginUser(HttpServletRequest request) {
        try {
            ...
            // 如果 Sa-Token 中没有，尝试�?Spring Session 中获取（兼容旧代码）
    ...
    public boolean changePassword(UserModifyPassWord userModifyPassWord, HttpServletRequest request) {
        ...
        if(userModifyPassWord.getNewPassword().length() < 8){
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "新密码长度不能小�?�?);
        }
        //查询是否有这个用�?
        QueryWrapper<User> queryWrapper = new QueryWrapper<>();
    ...
    public Map<String, String> getCaptcha() {
        ...
        // 将加密后的验证码�?Base64 编码的图片存储到 Redis 中，设置过期时间�?5 分钟�?00 秒）
    ...
    public boolean validateCaptcha(String userInputCaptcha, String serververifycode) {
        ...
        throw new BusinessException(ErrorCode.PARAMS_ERROR, "验证码错�?);
    }
    ...
    public void sendEmailCode(String email, String type, HttpServletRequest request) {
        ...
        if (ipCount != null && Integer.parseInt(ipCount) >= 5) {
            throw new BusinessException(ErrorCode.TOO_MANY_REQUEST, "请求验证码过于频繁，请稍后再�?);
        }
    ...
    public boolean changeEmail(String newEmail, String code, HttpServletRequest request) {
        ...
        synchronized (newEmail.intern()) {
            ...
            long count = this.baseMapper.selectCount(queryWrapper);
            if (count > 0) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "该邮箱已被使�?);
            }
    ...
    public boolean banOrUnbanUser(Long userId, Boolean isUnban, User admin) {
        ...
        if (!UserConstant.ADMIN_ROLE.equals(admin.getUserRole())) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR, "非管理员不能执行此操�?);
        }
        ...
        if (isUnban == isBanned) {
            ...
            throw new BusinessException(ErrorCode.OPERATION_ERROR,
                    String.format("该用户当�?s不需�?s", isUnban ? "未被封禁" : "已被封禁", operation));
        }
    }
    ...
    public void exportUserData(UserExportRequest exportRequest, HttpServletRequest httpRequest,
        ...
        try (HSSFWorkbook workbook = new HSSFWorkbook()) {
            ...
            // 2. 创建表头
            String[] headers = {"用户ID", "账号", "邮箱", "昵称", "角色", "创建时间", "状�?};
