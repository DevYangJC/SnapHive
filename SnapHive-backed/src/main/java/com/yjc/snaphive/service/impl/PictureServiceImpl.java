...
import com.yjc.snaphive.api.aliyunai.AliYunAiApi;
import com.yjc.snaphive.api.aliyunai.model.CreateOutPaintingTaskRequest;
import com.yjc.snaphive.api.aliyunai.model.CreateOutPaintingTaskResponse;
...
import com.yjc.snaphive.esdao.EsPictureDao;
...
import com.yjc.snaphive.manager.CosManager;
import com.yjc.snaphive.manager.CrawlerManager;
import com.yjc.snaphive.manager.auth.SpaceUserAuthManager;
import com.yjc.snaphive.manager.auth.model.SpaceUserPermissionConstant;
import com.yjc.snaphive.manager.upload.FilePictureUpload;
import com.yjc.snaphive.manager.upload.PictureUploadTemplate;
import com.yjc.snaphive.manager.upload.UrlPictureUpload;
import com.yjc.snaphive.mapper.ChatMessageMapper;
import com.yjc.snaphive.mapper.PictureMapper;
...
import com.yjc.snaphive.model.dto.picture.*;
import com.yjc.snaphive.model.entity.*;
import com.yjc.snaphive.model.entity.es.EsPicture;
import com.yjc.snaphive.model.enums.OperationEnum;
...
import com.yjc.snaphive.service.*;
import com.yjc.snaphive.utils.ColorSimilarUtils;
...
import com.yjc.snaphive.manager.auth.StpKit;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    public void validPicture(Picture picture) {
        ...
        if (StrUtil.isNotBlank(introduction)) {
            ThrowUtils.throwIf(introduction.length() > 800, ErrorCode.PARAMS_ERROR, "简介过�?);
        }
    }
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        if (pictureId != null) {
            ...
            ThrowUtils.throwIf(oldPicture == null, ErrorCode.NOT_FOUND_ERROR, "图片不存�?);
            // 校验空间是否一�?
            // 没传 spaceId，则复用原有图片�?spaceId（这样也兼容了公共图库）
            if (spaceId == null) {
                if (oldPicture.getSpaceId() != null) {
                    spaceId = oldPicture.getSpaceId();
                }
            } else {
                // 传了 spaceId，必须和原图片的空间 id 一�?
                if (ObjUtil.notEqual(spaceId, oldPicture.getSpaceId())) {
                    throw new BusinessException(ErrorCode.PARAMS_ERROR, "空间 id 不一�?);
                }
            }
        }
        // 上传图片，得到图片信�?
    ...
    public PictureVO getPictureVO(Picture picture, HttpServletRequest request) {
        ...
        if (loginUser != null) {
            // 使用 LikeRecordService 的方法来检查点赞状�?
    ...
    public void fillReviewParams(Picture picture, User loginUser) {
        if (userService.isAdmin(loginUser)) {
            ...
            picture.setReviewMessage("管理员自动过�?);
            picture.setReviewTime(new Date());
        } else {
            // 非管理员，无论是编辑还是创建默认都是待审�?
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        ThrowUtils.throwIf(count > 30, ErrorCode.PARAMS_ERROR, "最�?30 �?);
        // 名称前缀默认等于搜索关键�?
    ...
    public CreateOutPaintingTaskResponse createPictureOutPaintingTask(CreatePictureOutPaintingTaskRequest createPictureOutPaintingTaskRequest, User loginUser) {
        ...
        Picture picture = Optional.ofNullable(this.getById(pictureId))
                .orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND_ERROR, "图片不存�?));
        // 创建扩图任务
    ...
    public Page<PictureVO> getFollowPicture(HttpServletRequest request, PictureQueryRequest pictureQueryRequest) {
        ...
        // 创建 QueryWrapper 筛选出 userId 在关注列表中的图�?
        QueryWrapper<Picture> queryWrapper = new QueryWrapper<>();
    ...
    public PictureVO getPictureVOById(long id, HttpServletRequest request) {
        ...
        if (cachedChatCount != null) {
            ...
            stringRedisTemplate.opsForValue().set(
                    ...
                    300 + RandomUtil.randomInt(0, 60), // 添加随机过期时间，避免缓存雪�?
                    TimeUnit.SECONDS
            );
    ...
    private void handleUserQuery(Long userId, User loginUser, QueryWrapper<Picture> queryWrapper) {
        ...
        ThrowUtils.throwIf(loginUser == null || !loginUser.getId().equals(userId),
                ErrorCode.NO_AUTH_ERROR, "只能查看自己的图�?);
    ...
    public boolean setPictureFeature(PictureFeatureRequest pictureFeatureRequest, User loginUser) {
        ThrowUtils.throwIf(!UserConstant.ADMIN_ROLE.equals(loginUser.getUserRole()),
                ErrorCode.NO_AUTH_ERROR, "仅管理员可设置精�?);

        Long pictureId = pictureFeatureRequest.getId();
        Picture picture = this.getById(pictureId);
        ThrowUtils.throwIf(picture == null, ErrorCode.NOT_FOUND_ERROR, "图片不存�?);
